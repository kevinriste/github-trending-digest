...
RUN_LOG="/home/flog99/github-trending-digest-log-runs/${FIRST_LOG_DATE}.log"

RUN_LOG="$RUN_LOG" bash /home/flog99/dev/github-trending-digest/process.sh $FIRST_LOG_DATE 2>&1
exit_code=${PIPESTATUS[0]}
if [ "$exit_code" -ne 0 ]; then
    debug_message="GitHub Trending Digest error: process.sh exit code $exit_code"
    if [ -f "$RUN_LOG" ]; then
...
PY
    ...
    echo "GitHub Trending Digest--Exit code error- sending notification with Gotify"
